...
interfaceEnName = "addPolicyAdmin"
...
urlPath = authenticationApiPaths[0]['addPolicyAdmin']
...
class Test_class():
    ...
    def test_main(self, case, description, httpParameters):
        ...
        if httpParameters["paramType"] == "string":
            ...
            urlPath3 = urlPath + "/5f475da0893a250006859b74/addPolicyAdmin?userObjectId=5f5f4230f2c9e5000681e882"
    ...
    def test_all(self, case, description, httpParameters):
        ...
        if httpParameters["paramType"] == "string":
            ...
            urlPath3 = urlPath + "/5f475da0893a250006859b74/addPolicyAdmin?userObjectId=882"
            postJ = httpApi.httpRequests(urlIP, urlPath3, httpParameters["httpType"])
        else:
            assert 0, '参数类型「' + httpParameters["paramType"] + '」无法处理！'
        allure.attach("{0}".format(postJ), "接口出参")  # 测试用例的出参
    ...
    def test_all(self, case, description, httpParameters):
        ...
        if httpParameters["paramType"] == "string":
            ...
            urlPath3 = urlPath + "/5f475da0893a250006859b74/addPolicyAdmin?userObjectId=5d301e8f9858bf3159b6be36"
            postJ = httpApi.httpRequests(urlIP, urlPath3, httpParameters["httpType"])
        else:
            assert 0, '参数类型「' + httpParameters["paramType"] + '」无法处理！'
        allure.attach("{0}".format(postJ), "接口出参")  # 测试用例的出参
