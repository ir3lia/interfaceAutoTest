...
interfaceEnName = "addauthenticationPolicies"
...
urlPath = authenticationApiPaths[0]['addauthenticationPolicies']
...
class Test_class():
    ...
    def test_main(self, case, description, httpParameters):
        ...
        if httpParameters["paramType"] == "string":
            ...
            urlPath3 = urlPath + "?desc=%E6%B5%8B%E8%AF%95&isTerminateInferior=false&policyName=%E6%B5%8B%E8%AF%95&priority=1"
    ...
    def test_all(self, case, description, httpParameters):
        ...
        if httpParameters["paramType"] == "string":
            ...
            urlPath3 = urlPath + "?desc=%E6%B5%8B%E8%AF%95&filteringContent=111&isTerminateInferior=false&policyName=%E6%B5%8B%E8%AF%95&priority=1"
            postJ = httpApi.httpRequests(urlIP, urlPath3, httpParameters["httpType"], parame=httpParameters["query"])
        else:
            assert 0, '参数类型「' + httpParameters["paramType"] + '」无法处理！'
        allure.attach("{0}".format(postJ), "接口出参")  # 测试用例的出参
