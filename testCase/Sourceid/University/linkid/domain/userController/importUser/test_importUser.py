...
testerName = 'liaofeifei'
controllerName = "UserController"
interfaceChName = "导入用户"
interfaceEnName = "linkidImporUser"

commonConfig = readYaml.get_config("common/config.yaml", "common")  # 返回config.yaml文件中的common数组的所有数据
linkidApiPaths = readYaml.get_apiPath("common/apiPaths/linkidPath.yaml")  # 返回linkidPath.yaml文件中的所有linkid接口路径

urlIP = commonConfig[0]["sidselfIP"]
urlPath = linkidApiPaths[0]['linkidImportUser']
...
class Test_savaTeacher():
    '''
    def setup_method(self):
        print("\n ==清理数据库==")
        mongoDataOpr.oprDatasToDB(dbJsonPath,dbName,"all","mainCase")

    def teardown_method(self):
        print("\n ==清理数据库==")
        mongoDataOpr.oprDatasToDB(dbJsonPath,dbName,"delete","mainCase")
    '''
    ...
    def test_main(self, case, description, httpParameters):
        ...
        allure.attach(json.dumps(httpParameters["query"], ensure_ascii=False), "接口入参",
                      allure.attachment_type.JSON)  # 测试用例的入参
